...
<p align="center">
  <code>本地可安装候选</code> · <code>尚待真实安装验收</code> · <code>公开预览中</code>
</p>

> Agent 指替你执行任务的 AI 助手。仓库已经有第一版 `SKILL.md`，但尚未在干净环境完成真实安装与自然项目验收，不能称为成熟版本。
...
> **现在有什么**
> 已形成第一版单包 Skill 候选、选择题协作、自动项目化、如流四档、接棒与 HELP 合同；真实安装、宿主适配和自然项目效果仍待验证。

> **还没证明什么**
> 目前不能宣称已经通过安装验收、兼容所有 Agent、适合所有项目，也不能把上游试运行当成 Halazi Skill 已经真实通过。
...
当前 V1 还没有推到远端，先不要照下面安装；等 [发布清单](RELEASE_CHECKLIST.md) 的“远端读回”和“干净安装”打勾后，只需两步：

1. 在 Codex 里说：`请从 lixinpeng1998/Halazi-skill 安装根目录 Skill，名称用 halazi-skill。`
2. 下一轮直接说：`用 Halazi 帮我，我想……`，或只说自己的愿望 / 烦恼。

不用再装 Matt、Ponytail、看板或一组分 Skill。已有增强能力时 Halazi 会先验证再调用；没有时用内置规则和本地最薄板继续。卡住或忘了进度，直接说“帮帮我”。其他 Agent 端必须各自完成真实适配验收后，才会在这里给出安装入口。
...
- `SKILL.md` 目前只是本地可安装候选，尚未完成干净环境安装、自然触发和停止验收。
...
- **下一步：** 在干净 Codex 环境完成安装、自然触发、简单任务不项目化和 HELP 恢复验收。
